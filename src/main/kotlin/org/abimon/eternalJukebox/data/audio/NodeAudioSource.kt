...
object NodeAudioSource: NodeSource(), IAudioSource {
    override val nodeHosts: Array<String> = when {
        audioSourceOptions.containsKey("NODE_HOST") -> arrayOf(audioSourceOptions["NODE_HOST"] as? String ?: throw IllegalArgumentException("${audioSourceOptions["NODE_HOST"]}  is not of type 'String' (is ${audioSourceOptions["NODE_HOST"]?.javaClass}"))
        audioSourceOptions.containsKey("NODE_HOSTS") -> {
        ...
        else -> throw IllegalArgumentException("No hosts assigned for NodeAudioSource")
    }
